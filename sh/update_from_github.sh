...
# Dossier dans le repo contenant tes scripts
SUBDIR="${SUBDIR:-sh}"

DEST="/sdcard/cfl_watch"
WORK="$HOME/.cache/cfl_watch_repo"

# Fichier spécial (entrypoint) à déployer dans DEST/tmp
...
# 1) Preview rsync pour sh/ -> DEST/
# -n: dry-run
# -c: checksum (plus fiable que taille/date)
# --itemize-changes: liste ce qui change
RSYNC_OUT="$(rsync -nrc --itemize-changes --delete \
  --exclude '.git/' --exclude '.github/' \
  --exclude '*.md' \
  "$SRC_DIR"/ "$DEST"/ || true)"

# Filtre lignes utiles
CHANGES_SH="$(echo "$RSYNC_OUT" | sed '/^$/d' || true)"

# 2) Check console.sh diff (si présent)
CHANGES_CONSOLE=""
...
# Normalise CRLF + chmod +x sur tous les .sh déployés
find "$DEST" -maxdepth 1 -type f -name '*.sh' -print0 2>/dev/null | while IFS= read -r -d '' f; do
  sed -i 's/\r$//' "$f" 2>/dev/null || true
  chmod +x "$f" 2>/dev/null || true
done

# Déploiement console.sh -> tmp/console.sh (si dispo)
if [ -n "$CONSOLE_SRC" ]; then
