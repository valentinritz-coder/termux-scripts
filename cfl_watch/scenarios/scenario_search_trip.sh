...
ui_wait_screen(){
  local screen="$1"
  case "$screen" in
    home) ui_wait_resid "home:start field" "$ID_START" "$WAIT_LONG" ;;
    suggestions) wait_results_ready "$WAIT_LONG" "$WAIT_POLL" || true; ui_refresh ;;
    search_ready) ui_wait_search_button "$WAIT_LONG" ;;
    *) warn "ui_wait_screen inconnu: $screen"; return 1 ;;
  esac
}
...
# 0) Attendre l'écran avec le champ start
ui_wait_resid "start field visible" "$ID_START" "$WAIT_LONG" || sleep_s 2
ui_refresh
ui_snap "01_home" "$SNAP_MODE"

# 1) START: focus champ start
ui_tap_any "start field" \
  ...
  "desc:Select start" \
|| true
ui_snap "02_after_tap_start" "$SNAP_MODE"

# 2) START: saisir + attendre suggestions
ui_type_and_wait_results "start" "$START_TEXT"
ui_snap "03_after_type_start" "$SNAP_MODE"

# 3) START: sélectionner suggestion
ui_pick_suggestion "start suggestion" "$START_TEXT"
ui_refresh
ui_snap "04_after_pick_start" "$SNAP_MODE"

# 4) DESTINATION: attendre apparition champ destination (par content-desc, car parfois sans ID)
ui_wait_desc_any "destination field visible" "$WAIT_LONG" "destination" "arrivée"
ui_snap "05_destination_visible" "$SNAP_MODE"

# 5) DESTINATION: tap champ destination (desc d'abord, id en fallback)
...
# 10) Après search: dump + snap fort
ui_refresh
ui_snap "10_after_search" 3

# Heuristique de succès (comme avant)
