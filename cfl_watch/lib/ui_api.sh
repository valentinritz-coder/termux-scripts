...
ui_scroll_down_soft() {
  ...
  local x=$(( w / 2 ))
  local y_start=$(( h * 66 / 100 ))
  local y_end=$(( h * 40 / 100 ))

  # geste court + lent = pas d'inertie
  maybe adb shell input swipe "$x" "$y_start" "$x" "$y_end" 350
}
...
ui_get_desc_by_resid() {
  # Usage:
  #   ui_get_desc_by_resid ":id/searchHeader_back"
  #   ui_get_desc_by_resid "searchHeader_back"
