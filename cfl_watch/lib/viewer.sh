...
for f in run_dir.glob("*.*"):
    if f.suffix.lower() not in {".png", ".xml"}:
        continue
    base = f.stem
    snap.setdefault(base, {})[f.suffix.lower()] = f.name
...
for i, base in enumerate(bases):
    ...
    body_parts = [f"<h2>{esc(base)}</h2>", '<div class="nav">']
    if prev_page:
        body_parts.append(f'<a href="{esc(prev_page)}">⟵ Prev</a>')
    body_parts.append('<a href="index.html">Index</a>')
    if next_page:
        body_parts.append(f'<a href="{esc(next_page)}">Next ⟶</a>')
