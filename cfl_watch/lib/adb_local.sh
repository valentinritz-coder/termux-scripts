...
adbd_restart(){
  su -c 'stop adbd; start adbd' >/dev/null 2>&1 \
  || su -c 'setprop ctl.restart adbd' >/dev/null 2>&1 \
  || true
}
...
show_status(){
  ...
  log "adb devices -l:"
  adb devices -l || true
  log "Tip: export ANDROID_SERIAL=$SERIAL"
}
...
start(){
  ...
  log "Redémarrage adbd"
  adbd_restart
  log "Attente adbd=running"
  wait_adbd_running || warn "adbd pas 'running' (on tente quand même)"

  adb kill-server >/dev/null 2>&1 || true
  adb_server_start

  log "adb connect (retry) -> ${SERIAL}"
  connect_retry || die "Connexion ADB échouée"
...
case "${1:-}" in
  ...
  *)
    ...
    echo "Env: ADB_TCP_PORT=37099 ADB_HOST=127.0.0.1 ADB_SERIAL=127.0.0.1:37099"
    exit 2
    ;;
esac
