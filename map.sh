...
# Raw hash for "did the UI change?" detection
raw_xml_hash() {
  local xml="$1"
  python - "$xml" <<'PY'
import sys, hashlib
...
explore() {
  ...
  while IFS=$'\t' read -r x y label; do
    ...
    # Back ONCE (only because we detected a change)
